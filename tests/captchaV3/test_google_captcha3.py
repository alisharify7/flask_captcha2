...
def test_load_captcha_in_html(
    client, googlecaptcha3, app, captcha3_template_conf
):
    ...
    captcha = app.template_context_processors[None][-1]()[
        "captcha"
    ].render_captcha(model_name="flask-captcha-v3", **captcha3_template_conf)
    assert isinstance(captcha, Markup)
    assert f'data-sitekey="{googlecaptcha3.PUBLIC_KEY}"' in captcha
    assert (
        f"class=\"g-recaptcha {captcha3_template_conf['css_class']}\""
        in captcha
    )
    ...
    hidden_badge_style = Markup(
        "<style>.grecaptcha-badge {visibility: hidden;}</style>"
    )
    assert hidden_badge_style in captcha


def test_captcha_enable_on(
    app, googlecaptcha3, client, captcha3_template_conf
):
    ...
    captcha = app.template_context_processors[None][-1]()[
        ...
        model_name="flask-captcha-v3", conf=captcha3_template_conf
    )
